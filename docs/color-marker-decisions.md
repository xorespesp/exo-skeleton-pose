...
| 스트림 점프 통보 | 프레임에 세대 번호를 실어 소비자가 대조 | **프레임 사이에 끼우는 리셋 마커** | 마커는 데이터와 같은 스레드·같은 순서로 흐르므로 순서가 곧 보증이다. 대조할 신원이 필요 없어 `sensor_frame` 도 `on_sensor_frameset_update` 도 안 건드리고, `on_sensor_stream_reset()` 이 이미 그 자리에 있었다 |
| seek 실행 위치 | 호출자 스레드에서 실행하고, 워커가 그 다음 프레임을 버림 | **워커가 실행** | 버리는 안은 "이 프레임이 이동 전인가 후인가" 라는 모호함을 만들어 놓고 탐지기를 다는 형태다. 소스를 읽는 스레드만 소스를 옮기면 모호함 자체가 없어 판별할 것도 버릴 것도 없고, 정지 중 스크럽이 특례 없이 나온다 |
| 반복 재생 되감기 | `mcap_record_player` 가 `fetch` 안에서 되감음 | **provider 가 EOF 에서 seek** | 플레이어 안에서 되감으면 점프가 아무에게도 안 보여서 트래커도 추정기도 그대로 넘어간다. 반복은 일시정지·속도와 같은 재생 정책이라 자리도 provider 다 |
...
- **"GUI 때문에 근본 구조에 코드스멜을 남기나?"** → 아니다. 스레드 간 래치는 GUI 가 아니라
  **검출이 프레임 스레드, 추정이 루프 스레드**라서 생긴다. GUI 표시를 전부 지워도
  `process_frame` / `try_get_*` 분리와 `latest_value_latch` 는 남는다.

## 3. 남아 있는 구멍 (의도적으로 안 고침)

**고친 항목은 표시를 남기지 말고 행을 지운다.** 남아 있는 것만 여기 있어야 읽는 사람이 헷갈리지
않는다. 무엇을 왜 그렇게 고쳤는지는 §1 에 결정으로 들어간다.
